...
# ---------- Flask Web App ----------
app = Flask(__name__)
app.secret_key = "frexy_secret_key_fixed"
...
<body class="flex items-center justify-center min-h-screen p-4">
    <div class="w-full max-w-md bg-[#0f0b20] p-8 rounded-2xl border border-purple-500/30 shadow-2xl">
        <h2 class="text-2xl font-bold text-center mb-6 text-purple-400">FREXY AUTH SYSTEM</h2>
    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-[#0f0b20] rounded-xl overflow-hidden border border-white/10">
            <table class="w-full text-left">
                ...
                <tbody>
                    {% for user, data in users.items() if user != "frexy" %}
...
<body class="p-4 max-w-xl mx-auto">
    <header class="text-center my-6">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-cyan-400 to-pink-500 text-transparent bg-clip-text">FREXY PANEL</h1>
        <div class="flex justify-center gap-4 mt-2">
            {% if is_admin %}
            <a href="/admin" class="text-xs text-yellow-400 border border-yellow-400/30 px-2 py-1 rounded">Admin Panel</a>
            {% endif %}
